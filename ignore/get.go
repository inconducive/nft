...
import (
	...
	"github.com/inconducive/nft/api"
)
...
func remain() {
	...
	results = *GetFloorPrices(api.APIkey, ca) // single For loop of all contract Addresses fetched via GET
	log.Println(results)
...
func GetFloorPrices(apikey string, ca []string) *[]string {
	for _, v := range ca {
